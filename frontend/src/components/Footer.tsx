    ...
    <footer className="border-t border-border/75 bg-black/80 pb-20 md:pb-0">
      ...
      <div className="hidden md:block max-w-[1120px] mx-auto px-6 py-8">
        <div className="grid grid-cols-3 gap-8 mb-6">
          ...
          <div>
            ...
            <p className="text-xs text-subtle leading-relaxed max-w-[240px]">
              Privacy-first creator subscriptions powered by zero-knowledge proofs on Aleo.
            </p>
          </div>
          {/* Navigation */}
          <div>
            <p className="text-xs font-medium text-muted mb-3 uppercase tracking-wider">Platform</p>
            <div className="space-y-2">
              <Link href="/explore" className="block text-xs text-subtle hover:text-white transition-colors">Explore Creators</Link>
              <Link href="/verify" className="block text-xs text-subtle hover:text-white transition-colors">Verify Access</Link>
              <Link href="/explorer" className="block text-xs text-subtle hover:text-white transition-colors">On-Chain Explorer</Link>
              <Link href="/dashboard" className="block text-xs text-subtle hover:text-white transition-colors">Creator Dashboard</Link>
              <Link href="/analytics" className="block text-xs text-subtle hover:text-white transition-colors">Platform Analytics</Link>
            </div>
          </div>
          {/* Resources */}
          <div>
            <p className="text-xs font-medium text-muted mb-3 uppercase tracking-wider">Resources</p>
            <div className="space-y-2">
      ...
      <div className="md:hidden px-4 py-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <a href="https://github.com/Pratiikpy/Veil-Sub" target="_blank" rel="noopener noreferrer" aria-label="GitHub repository" className="flex items-center gap-1 text-xs text-subtle hover:text-muted transition-colors">
            <Github className="w-3.5 h-3.5" aria-hidden="true" />
          </a>
        </div>
        <div className="flex flex-wrap gap-x-4 gap-y-1.5 mb-4">
          <Link href="/docs" className="text-xs text-subtle hover:text-white transition-colors">Docs</Link>
          <Link href="/privacy" className="text-xs text-subtle hover:text-white transition-colors">Privacy</Link>
          <Link href="/vision" className="text-xs text-subtle hover:text-white transition-colors">Vision</Link>
          <Link href="/explorer" className="text-xs text-subtle hover:text-white transition-colors">On-Chain</Link>
        </div>
