...
export default function ExplorerPage() {
  ...
  useEffect(() => {
    ...
    setStatsError(false)
    fetch('/api/analytics?global_stats=true', { signal: controller.signal })
      .then((r) => r.json())
      .then(setGlobalStats)
      .catch((err) => {
  ...
  useEffect(() => {
    ...
    fetch('/api/analytics?recent=true', { signal: controller.signal })
      .then((r) => r.json())
      .then((json) => {
    ...
    <PageTransition>
      <div className="min-h-screen">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          ...
          {!globalStats && !statsError && (
            <div className="mb-10">
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                {[1,2,3,4].map(i => <div key={i} className="h-20 rounded-xl bg-white/[0.03] animate-pulse" />)}
              </div>
            </div>
          )}
          {statsError && !globalStats && (
            <div className="mb-10">
              <div className="px-4 py-3 rounded-lg bg-amber-500/10 border border-amber-500/20 text-sm text-amber-300 flex items-center justify-between gap-2">
                <div className="flex items-center gap-2">
                  <AlertCircle className="w-4 h-4 shrink-0" aria-hidden="true" />
                  <span>Stats temporarily unavailable. The analytics API could not be reached.</span>
                </div>
                <button
                  onClick={() => {
                    setStatsError(false)
