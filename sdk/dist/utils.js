...
exports.stripLeoSuffix = stripLeoSuffix;
...
exports.microToCredits = microToCredits;
exports.creditsToMicro = creditsToMicro;
...
function toU8(n) { return `${n}u8`; }
/** Format a number as Leo u32 */
function toU32(n) { return `${n}u32`; }
/** Format a number as Leo u64 */
function toU64(n) { return `${n}u64`; }
...
function isValidAleoAddress(address) {
    return constants_1.ALEO_ADDRESS_REGEX.test(address);
}
/**
...
function assertValidAddress(address, label = 'address') {
    if (!isValidAleoAddress(address)) {
        throw new types_1.VeilSubError(`Invalid Aleo ${label}: "${address}". Must match pattern aleo1[a-z0-9]{58}`);
    }
}
// ---------------------------------------------------------------------------
// Unit Conversions
 ...
 */
function microToCredits(microcredits) {
    return microcredits / constants_1.MICROCREDITS_PER_CREDIT;
}
/**
 ...
 */
function creditsToMicro(credits) {
    return Math.round(credits * constants_1.MICROCREDITS_PER_CREDIT);
}
/**
 ...
 */
function calculateExpiry(currentBlockHeight, durationBlocks = 864000) {
