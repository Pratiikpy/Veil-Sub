...
export class SubscriptionMonitor {
    ...
    creatorSnapshots = new Map();
    platformSnapshot = { totalCreators: 0, totalContent: 0, blockHeight: 0 };
    isFirstPoll = true;
    // Lifecycle
    running = false;
    pollTimer = null;
    ...
    // ── Platform Stats ────────────────────────────────────────────────────
    async pollPlatformStats(blockHeight) {
    ...
    // ── Creator Polling ───────────────────────────────────────────────────
    async pollCreator(creator, blockHeight) {
        ...
        if (this.isFirstPoll && (current.subscriberCount > 0 || current.totalRevenue > 0)) {
            logInfo(`  ${creator.label}: ${current.subscriberCount} subs, ` +
